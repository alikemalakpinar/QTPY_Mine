...
class PersonDetailScreen(QWidget):
    ...
    def create_header(self):
        ...
        back_btn.setStyleSheet(AicoTheme.get_button_style('primary'))
        back_btn.clicked.connect(self.back_requested.emit)
        
        title_layout = QVBoxLayout()
        title = QLabel(f"👤 {self.person['full_name']}")
        title.setStyleSheet(f"font-size: 32px; font-weight: 700; color: {AicoTheme.TEXT_PRIMARY};")
        
        subtitle = QLabel(self.person['position'])
        subtitle.setStyleSheet(f"font-size: 16px; color: {AicoTheme.TEXT_SECONDARY}; margin-top: 5px;")
    ...
    def create_status_badge(self):
        ...
        status_colors = {
            'active': (AicoTheme.SUCCESS, '🟢 Aktif'),
            'break': (AicoTheme.WARNING, '🟡 Molada'),
            'emergency': (AicoTheme.DANGER, '🔴 Acil Durum')
        }
        
        color, text = status_colors.get(status, (AicoTheme.TEXT_SECONDARY, 'Bilinmiyor'))
    ...
    def create_profile_card(self):
        card = QWidget()
        card.setStyleSheet(AicoTheme.get_card_style(hover=False))
        layout = QVBoxLayout(card)
        layout.setSpacing(15)
        
        title = QLabel("📋 Profil Bilgileri")
        title.setStyleSheet(f"font-size: 18px; font-weight: 700; color: {AicoTheme.TEXT_PRIMARY};")
        ...
        for label, value in info_items:
            ...
            label_widget.setStyleSheet(f"color: {AicoTheme.TEXT_SECONDARY}; font-size: 13px;")
            
            value_widget = QLabel(str(value))
            value_widget.setStyleSheet(f"color: {AicoTheme.TEXT_PRIMARY}; font-size: 14px; font-weight: 600;")
    ...
    def create_location_card(self):
        ...
        zone_label.setStyleSheet(f"font-size: 16px; font-weight: 600; color: {AicoTheme.PRIMARY};")
        ...
        for axis, value in [('X', loc['x']), ('Y', loc['y']), ('Z', loc['z'])]:
            ...
            axis_label.setStyleSheet(f"color: {AicoTheme.TEXT_SECONDARY}; font-size: 12px;")
            axis_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
            
            value_label = QLabel(f"{value:.1f}m")
            value_label.setStyleSheet(f"color: {AicoTheme.TEXT_PRIMARY}; font-size: 18px; font-weight: 700;")
        ...
        self.last_update_label.setStyleSheet(f"color: {AicoTheme.TEXT_MUTED}; font-size: 12px;")
    ...
    def create_metric_widget(self, icon, label, value, unit):
        ...
        label_widget = QLabel(label)
        label_widget.setStyleSheet(f"color: {AicoTheme.TEXT_SECONDARY}; font-size: 12px;")
        label_widget.setAlignment(Qt.AlignmentFlag.AlignCenter)
        
        value_widget = QLabel(f"{value}{unit}")
        value_widget.setStyleSheet(f"color: {AicoTheme.TEXT_PRIMARY}; font-size: 20px; font-weight: 700;")
    ...
    def create_time_tracking_card(self):
        ...
        self.work_duration_label.setStyleSheet(f"font-size: 32px; font-weight: 700; color: {AicoTheme.PRIMARY};")
        ...
        work_label.setStyleSheet(f"color: {AicoTheme.TEXT_SECONDARY}; font-size: 13px;")
        work_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        layout.addWidget(work_label)
        
        divider = QFrame()
        divider.setFrameShape(QFrame.Shape.HLine)
        divider.setStyleSheet(f"background: {AicoTheme.BORDER};")
        ...
        self.break_time_label.setStyleSheet(f"font-size: 20px; font-weight: 700; color: {AicoTheme.WARNING};")
        self.break_time_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        
        break_label = QLabel("Mola Süresi")
        break_label.setStyleSheet(f"color: {AicoTheme.TEXT_SECONDARY}; font-size: 12px;")
        ...
        self.active_time_label.setStyleSheet(f"font-size: 20px; font-weight: 700; color: {AicoTheme.SUCCESS};")
        self.active_time_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        
        active_label = QLabel("Aktif Çalışma")
        active_label.setStyleSheet(f"color: {AicoTheme.TEXT_SECONDARY}; font-size: 12px;")
    ...
    def create_activity_item(self, icon, action, time):
        ...
        action_label.setStyleSheet(f"color: {AicoTheme.TEXT_PRIMARY}; font-size: 13px; font-weight: 500;")
        
        time_label = QLabel(time)
        time_label.setStyleSheet(f"color: {AicoTheme.TEXT_MUTED}; font-size: 11px;")
